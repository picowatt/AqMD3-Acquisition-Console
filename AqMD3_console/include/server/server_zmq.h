...
public:
	...
	public:
		ReceivedRequest(ServerZmq& server, std::string sender, std::vector<std::string> payload)
			: server(server)
			, sender_id(sender)
			, payload(payload)
		{}
	...
	public:
		...
		bool publish(zmq::message_t& message, const std::string& subject, std::chrono::milliseconds timeout)
		{
			bool r = true;
