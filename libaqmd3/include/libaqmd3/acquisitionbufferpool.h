...
namespace aqmd3
{
	...
	public:
		AcquisitionBufferPool(uint64_t max_triggers_per_read, uint64_t samples_per_trigger, uint64_t buffer_initial_count, uint64_t buffer_max_count)
			: buffer_takes(0), buffer_max_count(buffer_max_count), buffer_initial_count(buffer_initial_count), max_triggers_per_read(max_triggers_per_read), samples_per_trigger(samples_per_trigger)
		{
