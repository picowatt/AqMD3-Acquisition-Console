...
namespace aqmd3
{
	...
	public:
		...
		public:
			...
			GateData(uint64_t gate_start_block_index, uint32_t gate_start_sample_pos, uint64_t gate_stop_block_index,
				...
				: gate_start_block_index(gate_start_block_index), gate_start_sample_pos(gate_start_sample_pos), gate_stop_block_index(gate_stop_block_index), gate_stop_sample_pos(gate_stop_sample_pos), element_count(element_count)
			{
			}
		...
		public:
			...
			TriggerData(uint64_t timestamp, uint32_t index, double subsample_pos)
				: timestamp(timestamp), index(index), subsample_pos(subsample_pos), gate_data()
			{
			}
		};
	...
	public:
		...
		AcquiredData(std::vector<TriggerData> stamps, std::shared_ptr<AcquisitionBuffer> samples_buffer, uint64_t samples_count)
			: samples_buffer(samples_buffer), stamps(stamps), samples_count(samples_count)
		{
		}
