...
public:
    ...
    static TofTimingInformation create_timing_information(const aqmd3::SA220P *digitizer, double sample_rate, double post_trigger_delay_seconds, double trigger_rearm_time_seconds);
    static std::tuple<uint64_t, uint64_t, uint64_t> get_optimal_record_size(const aqmd3::SA220P *digitizer, uint64_t pusher_pulse_pulse_width_samples, double post_trigger_delay_s, double sample_rate, double trig_rearm_s);
    static uint64_t get_trigger_time_stamp_average(const aqmd3::SA220P *digitizer, int triggers);
